...
| `require_t03_frontier_consumer` | `T03` | `src/substrate/subject_tick/update.py` | `rt01.t03_hypothesis_competition_checkpoint` | `derive_t03_preverbal_competition_consumer_view(...).frontier_consumer_ready` | `repair` detour | `test_t03_downstream_preverbal_consumer_is_load_bearing` (package-level), no dedicated `subject_tick` test name | INFERRED |
