    ...
    return W02InputBundle(
        ...
        traces=(
            W02TraceRef(
                ...
                provenance_ref=("tests.w05.runtime", case_id),
                action_ref="action:a",
                effect_ref="effect:a",
        ...
        source_lineage=("tests.w05.runtime", case_id),
        reason="w05 runtime integration fixture",
    )


def test_w05_runtime_topology_contains_checkpoint_and_surface() -> None:
    graph = build_minimal_runtime_tick_graph()
    assert "rt01.w05_predictive_prior_injection_checkpoint" in graph.mandatory_checkpoint_ids
    assert "w05_predictive_prior_injection.routing_result" in graph.source_of_truth_surfaces


def test_w05_is_after_w04_before_m01() -> None:
    case_id = "runtime-topology-w05-order"
    w02_input = _w02_input(case_id)
    w03_input = w03_input_from_w02(
        case_id=case_id,
        w02_input=w02_input,
        source_lineage=("tests.w05.runtime", case_id),
    )
...
def test_w05_disable_flag_rejected_in_production_route() -> None:
    case_id = "runtime-topology-w05-disabled"
...
def test_world_to_narrative_chain_order_remains_stable_after_w05() -> None:
    case_id = "runtime-topology-w05-chain-order"
    w02_input = _w02_input(case_id)
    w03_input = w03_input_from_w02(
        case_id=case_id,
        w02_input=w02_input,
