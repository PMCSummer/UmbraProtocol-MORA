    ...
    return a01_candidate_set(
        ...
        candidates=(
            a01_candidate(
                ...
                aliases=(),
                provenance=f"tests.a04.runtime:{case_id}:c1",
...
def _a04_candidate_set(case_id: str, *, authority_missing: bool = False) -> A04ExternalAffordanceCandidateSet:
    return A04ExternalAffordanceCandidateSet(
        ...
        candidates=(
            A04ExternalAffordanceCandidate(
                ...
                confidence=0.8,
                provenance=("tests.a04.runtime", case_id),
            ),
        ),
        world_scaffolds=(
            A04WorldEntityScaffold(
                ...
                confidence=0.85,
                temporal_validity="valid_now",
                provenance=("tests.a04.runtime.scaffold", case_id),
                supported_affordance_classes=("world_directed_action",),
                object_ref=f"object:{case_id}",
            ),
        ),
        source_lineage=("tests.a04.runtime", case_id),
        reason="a04 runtime fixture",
    )


def test_runtime_topology_graph_includes_a04_checkpoint_and_surface() -> None:
...
def test_dispatch_a04_require_path_is_enforced() -> None:
    case_id = "runtime-topology-a04-require"
    result = dispatch_runtime_tick(
        RuntimeDispatchRequest(
            ...
            context=SubjectTickContext(
                ...
                require_a04_authority_path_consumer=True,
            ),
    ...
    assert "require_a04_authority_path_consumer" in checkpoint.required_action
    assert result.subject_tick_result.state.final_execution_outcome == SubjectTickOutcome.REVALIDATE


def test_dispatch_a04_integration_indispensability_rejects_gate_disabled_fixture_on_production_route() -> None:
    case_id = "runtime-topology-a04-integration-indispensability"
