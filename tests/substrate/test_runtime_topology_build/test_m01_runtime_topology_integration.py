    ...
    return a01_candidate_set(
        ...
        candidates=(
            a01_candidate(
                ...
                aliases=(),
                provenance=f"tests.m01.runtime:{case_id}:c1",
    ...
    return A04ExternalAffordanceCandidateSet(
        ...
        candidates=(
            A04ExternalAffordanceCandidate(
                ...
                confidence=0.8,
                provenance=("tests.m01.runtime", case_id),
            ),
        ),
        world_scaffolds=(
            A04WorldEntityScaffold(
                ...
                confidence=0.85,
                temporal_validity="valid_now",
                provenance=("tests.m01.runtime.scaffold", case_id),
                supported_affordance_classes=("world_directed_action",),
                object_ref=f"object:{case_id}",
            ),
        ),
        source_lineage=("tests.m01.runtime", case_id),
        reason="m01 runtime fixture",
    )
    ...
    return W01WorldPacketSet(
        ...
        packets=(
            W01WorldPacket(
                ...
                provenance_ref=("tests.m01.runtime", case_id),
        ...
        source_lineage=("tests.m01.runtime", case_id),
        reason="m01 runtime packet fixture",
    )


def _m01_input(case_id: str, *, with_delta: bool):
    trace = m01_trace(trace_id=f"{case_id}:trace")
    deltas = ()
    coupling = ()
    if with_delta:
        delta = m01_delta(
    ...
    return m01_bundle(
        ...
        deltas=deltas,
        coupling=coupling,
        attribution=(m01_attribution(trace_id=trace.trace_id, attribution_status=M01AttributionStatus.SELF_RELEVANT),),
        source_lineage=("tests.m01.runtime", case_id),
        reason=case_id,
    )


def test_runtime_topology_graph_includes_m01_checkpoint_and_surface() -> None:
...
def test_dispatch_m01_checkpoint_order_is_after_w01_before_outcome_resolution() -> None:
    case_id = "runtime-topology-m01-order"
    result = dispatch_runtime_tick(
        RuntimeDispatchRequest(
            ...
            context=SubjectTickContext(
                ...
                m01_input_bundle=_m01_input(case_id, with_delta=True),
            ),
...
def test_dispatch_m01_require_path_is_enforced() -> None:
    case_id = "runtime-topology-m01-require"
    result = dispatch_runtime_tick(
        RuntimeDispatchRequest(
            tick_input=_tick_input(case_id),
            context=SubjectTickContext(
                ...
                a04_external_candidate_set=_a04_candidate_set(case_id),
                w01_world_packet_set=_w01_packet_set(case_id),
                m01_input_bundle=_m01_input(case_id, with_delta=False),
    ...
    assert "require_m01_imprint_packet_consumer" in checkpoint.required_action
    assert result.subject_tick_result.state.final_execution_outcome == SubjectTickOutcome.REVALIDATE


def test_dispatch_production_route_rejects_m01_gate_disabled_fixture() -> None:
    case_id = "runtime-topology-m01-disabled"
