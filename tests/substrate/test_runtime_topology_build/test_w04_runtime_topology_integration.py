    ...
    return W02InputBundle(
        ...
        traces=(
            W02TraceRef(
                ...
                provenance_ref=("tests.w04.runtime", case_id),
                action_ref="action:a",
                effect_ref="effect:a",
        ...
        source_lineage=("tests.w04.runtime", case_id),
        reason="w04 runtime integration fixture",
    )


def test_w04_runtime_topology_contains_checkpoint_and_surface() -> None:
    graph = build_minimal_runtime_tick_graph()
    assert "rt01.w04_applicability_gating_checkpoint" in graph.mandatory_checkpoint_ids
    assert "w04_applicability_gating.applicability_result" in graph.source_of_truth_surfaces


def test_w04_is_after_w03_before_m01() -> None:
    case_id = "runtime-topology-w04-order"
...
def test_w04_disable_flag_rejected_in_production_route() -> None:
    case_id = "runtime-topology-w04-disabled"
...
def test_world_to_narrative_chain_order_remains_stable_after_w04() -> None:
    case_id = "runtime-topology-w04-chain-order"
    w02_input = _w02_input(case_id)
    w03_input = w03_input_from_w02(
        case_id=case_id,
        w02_input=w02_input,
