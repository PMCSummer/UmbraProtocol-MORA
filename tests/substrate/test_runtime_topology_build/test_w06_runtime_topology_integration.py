    ...
    return W02InputBundle(
        ...
        traces=(
            W02TraceRef(
                ...
                provenance_ref=("tests.w06.runtime", case_id),
                action_ref="action:a",
                effect_ref="effect:a",
        ...
        source_lineage=("tests.w06.runtime", case_id),
        reason="w06 runtime integration fixture",
    )


def test_w06_runtime_topology_contains_checkpoint_and_surface() -> None:
    graph = build_minimal_runtime_tick_graph()
    assert "rt01.w06_error_driven_revision_checkpoint" in graph.mandatory_checkpoint_ids
    assert "w06_error_driven_revision.revision_result" in graph.source_of_truth_surfaces


def test_w06_is_after_w05_before_m01() -> None:
    case_id = "runtime-topology-w06-order"
...
def test_w06_disable_flag_rejected_in_production_route() -> None:
    case_id = "runtime-topology-w06-disabled"
...
def test_world_to_narrative_chain_order_remains_stable_after_w06() -> None:
    case_id = "runtime-topology-w06-chain-order"
    w02_input = _w02_input(case_id)
    w03_input = w03_input_from_w02(
        case_id=case_id,
        w02_input=w02_input,
