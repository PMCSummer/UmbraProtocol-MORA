...
def test_g07_requires_typed_g05_and_g06_upstream_only(g07_factory) -> None:
    ctx = g07_factory("he said that you are tired", "g07-boundary")
    with pytest.raises(TypeError):
        build_targeted_clarification("raw text", ctx.framing)
    with pytest.raises(TypeError):
        build_targeted_clarification(ctx.acquisition, "raw framing")
    with pytest.raises(TypeError):
...
def test_insufficient_basis_forces_abstain(g07_factory) -> None:
    ...
    result = build_targeted_clarification(ctx.acquisition, degraded)
