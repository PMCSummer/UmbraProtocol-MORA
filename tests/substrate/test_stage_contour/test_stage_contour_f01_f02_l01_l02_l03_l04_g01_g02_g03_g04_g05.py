...
def test_stage_contour_f01_f02_l01_l02_l03_l04_g01_g02_g03_g04_g05_preserves_single_write_seam() -> None:
    boot = execute_transition(
        TransitionRequest(
            transition_id="tr-g05-stage-boot",
            transition_kind=TransitionKind.BOOTSTRAP_INIT,
            writer=WriterIdentity.BOOTSTRAPPER,
            cause_chain=("bootstrap",),
            requested_at="2026-04-04T00:00:00+00:00",
            event_id="ev-g05-stage-boot",
    ...
    epistemic = ground_epistemic_input(
        InputMaterial(material_id="m-g05-stage", content='he said "you are tired?"'),
        SourceMetadata(
            source_id="user-g05-stage",
    ...
    lexical_result = build_lexical_grounding_hypotheses(
        ...
        discourse_context=LexicalDiscourseContext(context_ref="ctx:g05-stage"),
    )
    ...
    grounded_result = build_grounded_semantic_substrate(
        dictum_result,
        utterance_surface=surface_result,
        memory_anchor_ref="m05:g05-stage",
        cooperation_anchor_ref="o05:g05-stage",
    )
    ...
    assert acquisition_result.bundle.acquisition_records
    assert acquisition_result.bundle.cluster_links
    assert acquisition_result.bundle.no_final_semantic_closure is True
    gate = evaluate_semantic_acquisition_downstream_gate(acquisition_result)
    assert "no_final_semantic_closure" in gate.restrictions
    assert "acquisition_status_must_be_read" in gate.restrictions
    assert "accepted_provisional_not_commitment" in gate.restrictions
    contract_view = derive_semantic_acquisition_contract_view(acquisition_result)
    assert contract_view.requires_status_read is True
    assert contract_view.requires_restrictions_read is True
    assert contract_view.accepted_provisional_not_commitment is True
    ...
    persisted = persist_semantic_acquisition_result_via_f01(
        ...
        transition_id="tr-g05-stage-persist",
...
def test_stage_contour_g05_typed_only_guard_no_raw_or_g04_bypass() -> None:
