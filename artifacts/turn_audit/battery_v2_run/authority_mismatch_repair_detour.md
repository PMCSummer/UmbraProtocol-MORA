# Turn summary
- artifact path: `artifacts\turn_audit\battery_v2_run\authority_mismatch_repair_detour.json` (field: input artifact file)
...
- final execution outcome: `repair` (field: final_outcome.final_execution_outcome)
...
- enforced detour checkpoint ids: rt01.critical_gate_checkpoint, rt01.world_entry_checkpoint, rt01.outcome_resolution_checkpoint (field: checkpoints.enforced_detour_checkpoint_ids)

Explicit checkpoint rows:
| Checkpoint | Status | Required action | Applied action | Reason |
| --- | --- | --- | --- | --- |
| `rt01.epistemic_admission_checkpoint` | `allowed` | `consume_epistemic_allowance_and_preserve_abstain_unknown_conflict_markers` | `reported_claim:continue_stream` | `report cannot be promoted to observation` |
| `rt01.shared_runtime_domain_checkpoint` | `allowed` | `consume_shared_regulation_continuity_validity_domains` | `continue_stream` | `no prior shared runtime state supplied; contour followed phase-local contracts` |
| `rt01.downstream_obedience_checkpoint` | `allowed` | `allow_continue` | `continue_stream` | `downstream obedience allows bounded continuation` |
| `rt01.outcome_resolution_checkpoint` | `enforced_detour` | `bounded_outcome_must_be_resolved` | `repair_path:repair_runtime_path` | `runtime contour resolved bounded outcome from enforced contracts` |
...
- abstain_reason: `repair_required` (field: uncertainty_and_fallbacks.abstain_reason)
...
- execution stance: `repair_path` (field: final_outcome.execution_stance)
- active execution mode: `repair_runtime_path` (field: final_outcome.active_execution_mode)
- repair_needed: `true` (field: final_outcome.repair_needed)
