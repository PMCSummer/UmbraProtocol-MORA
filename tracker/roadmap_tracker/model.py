...
RELATION_CONTRACT_HEADER = "## RELATION SEMANTIC CONTRACT"
RELATION_CONTRACT_CODE_RE = re.compile(r"^[A-Z][A-Z0-9.]*$")
...
def parse_relation_contract_code_token(raw: str | None) -> str:
    token = str(raw or "").strip()
    if not token:
        return ""
    head = token.split()[0].strip()
    return head if RELATION_CONTRACT_CODE_RE.match(head) else ""


def parse_relation_contract_pairs_from_text(text: str) -> Dict[tuple[str, str], str]:
    if RELATION_CONTRACT_HEADER not in str(text or ""):
        return {}
    section = str(text or "").split(RELATION_CONTRACT_HEADER, 1)[1]
    arrow = "->"
    ...
    for raw_line in section.splitlines():
        ...
        if not line.startswith("- "):
            continue
        if arrow not in line or ": `" not in line:
            continue
        left, right = line[2:].split(arrow, 1)
        targets, tail = right.strip().split(": `", 1)
        relation = tail.split("`", 1)[0].strip()
        relation = normalize_edge_relation(relation)
        src_codes = [parse_relation_contract_code_token(token) for token in left.split("/")]
        tgt_codes = [parse_relation_contract_code_token(token) for token in targets.split("/")]
        src_codes = [token for token in src_codes if token]
        tgt_codes = [token for token in tgt_codes if token]
        for source_code in src_codes:
            for target_code in tgt_codes:
                pairs[(source_code, target_code)] = relation
    return pairs
...
class RoadmapModel:
    ...
    def rebuild_phase_bindings(self) -> None:
        ...
        autogenerated_workflow_pairs = {
            (f"phase::{dep}", f"phase::{normalize_code(phase.code)}")
            for phase in self.phases
        ...
        self.graph_edges = [
            ...
            if not (
                ...
                (
                    (
                        edge.relation == EDGE_RELATION_DEFAULT and
                        edge.layer == "workflow" and
                        (edge.source, edge.target) in autogenerated_workflow_pairs
                    )
    ...
    def seam_relation_contract_pairs(
        self,
        *,
        seam_dir: Optional[Path] = None,
        seam_files: Optional[Iterable[str]] = None,
    ) -> Dict[tuple[str, str], str]:
        if seam_dir is None:
            seam_dir = Path(__file__).resolve().parents[2] / "docs" / "seams"
        if seam_files is None:
            files = sorted(path.name for path in seam_dir.glob("*.seam.md"))
        else:
            files = [str(name).strip() for name in seam_files if str(name).strip()]

        pairs: Dict[tuple[str, str], str] = {}
        for filename in files:
            seam_path = seam_dir / filename
            if not seam_path.exists():
                continue
            text = seam_path.read_text(encoding="utf-8", errors="ignore")
            pairs.update(parse_relation_contract_pairs_from_text(text))
        return pairs

    def seam_relation_contract_violations(
        self,
        *,
        seam_dir: Optional[Path] = None,
        seam_files: Optional[Iterable[str]] = None,
    ) -> List[str]:
        declared_pairs = self.seam_relation_contract_pairs(seam_dir=seam_dir, seam_files=seam_files)
        relation_by_pair: Dict[tuple[str, str], str] = {}
        for edge in self.graph_edges:
            source_code = parse_phase_node_code(edge.source)
            target_code = parse_phase_node_code(edge.target)
            if not source_code or not target_code:
                continue
            relation_by_pair[(source_code, target_code)] = normalize_edge_relation(edge.relation)

        violations: List[str] = []
        for pair, expected in sorted(declared_pairs.items()):
            actual = relation_by_pair.get(pair)
            if actual is None:
                violations.append(f"{pair[0]}->{pair[1]} missing_edge expected={expected}")
                continue
